...
if [ "$el_version" = "6" ]; then
	...
	docker run --privileged -d -t -e "container=docker"  -v /sys/fs/cgroup:/sys/fs/cgroup -v `pwd`:/home/travis/build/chvalean/lis-next:rw  ${CENTOS}   tail -f /dev/null
	...
	docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "yum clean all"
	...
	docker exec -ti $DOCKER_CONTAINER_ID /bin/bash -xec "bash -e /home/travis/build/chvalean/lis-next/hv-rhel7.x/hv/rhel7-hv-driver-install"
