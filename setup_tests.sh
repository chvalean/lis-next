...
if [ "$el_version" = "6" ]; then
	...
	docker run --privileged --exec-opt native.cgroupdriver=cgroupfs -d -ti -e "container=docker"  -v /sys/fs/cgroup:/sys/fs/cgroup -v `pwd`:/home/travis/build/chvalean/lis-next:rw  ${CENTOS}   /usr/sbin/init
